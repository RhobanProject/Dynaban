...
//--------------------------------------------  
  ...
  rep=get_absolute_file_path("seance2_exemple_av_phase.sce");chdir(rep); // on se place sous le repertoire contenant le fichier 'seance2_exemple_av_phase.sce'
...
//---------------------------------------------------
  fe=50;   // frequence d'echantillonnage en hertz
  te=1/fe; // periode d'echantillonnage en secondes
//-----------------------------------
// 3- definition de F(w) ( correcteur avance de phase)
//-----------------------------------
  phi_u_rad=50*%pi/180;Gu=10; // phase et gain du regulateur
  wu_reelle=10;               // pulsation wu reelle en rad/s
  vu=tan(wu_reelle*te/2); // pseudo-pulsation vu associee a wu_reelle
  a=tan((phi_u_rad+%pi/2)/2); // parametre a
  c0=1/a;                     // gain statique c0
  v0=vu/a;
  v1=a*vu;
  F_de_w=c0*(1+w/v0)/(1+w/v1); // fonction de transfert en w du regulateur
