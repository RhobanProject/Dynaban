...
//--------------------------------------------  
  ...
  rep=get_absolute_file_path("band_pass_iir_example.sce");chdir(rep); // on se place sous le repertoire contenant le fichier 'fir.sce'
...
//---------------------------------------------------
  f0=100; // stopband low  Frequency f0 (Hz)
  f1=200; // bandpass low  Frequency f1 (Hz)
  f2=500; // bandpass high Frequency f2 (Hz)
  f3=700; // stopband high Frequency f3 (Hz)
  G03_db=20*log10(0.01);  // maximum Gain for f<=f0 or f>=f3, decibels
  G12_db=20*log10(0.95); // minimum Gain for f1<=f<=f2, decibels
//-----------------------------------------------------
// 2- filter model
...
//------------------------------------------------------
   params.file_name="band_pass"; // will generate toto.c,toto_int.c,toto_float.c
//---------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
   params.name_filter="band_pass"; // specifc functions for this filter will end with _Fz
//--------------------------------------------------------------
