// this scilab programme generate c code of an iir band-stop filter
// read remarks to adapt it to your own problem
   clear
   lines(0);
   rep=get_absolute_file_path("band_stop_iir_example.sce");
   chdir(rep);
//-------------------------------------------------------------
// 1-FILTER DEFINITION ( MODIFIY AS YOUR CONVENIENCE)
//-------------------------------------------------------------
//-------------------------------------------------
...
//---------------------------------------------------
// 1-band stop Filter template in Real Frequencies (Hz))
//---------------------------------------------------
  f0=30; // stopband low  Frequency f0 (Hz)
  f1=45; // bandpass low  Frequency f1 (Hz)
  f2=55; // bandpass high Frequency f2 (Hz)
  f3=70; // stopband high Frequency f3 (Hz)
  G03_db=20*log10(0.99);  // minimum Gain for f<=f0 or f>=f3, decibels
  G12_db=20*log10(0.01);  // maximum Gain for f1<=f<=f2, decibels
...
//------------------------------------------------------------------------
  modele_flt="cheb1"; // "butt","cheb1","cheb2","ellip"
//-----------------------------------------------------
// 2-CHOOSE FREQUENCY RESPONSE DRAWING DOMAIN
//------------------------------------------------------
  f0_aff=0; // min frequency for drawing
  f1_aff=min([10*f1,0.49*fs]); // max frequency for drawing
  nb_points_aff=1000; // number of drawing pointsround
//-----------------------------------------------------
// 3-code generation parameters
//----------------------------------------------------
//-------------------------------------------------------------------
...
// only the l1 norm scaling e(default) ensures that overflow will never occur...
//-------------------------------------------------------------------
...
// it is automatically setted by the software if you don't specify it
//------------------------------------------------------------------------------
...
// for exemple
//   params.file_name="toto"; will generate one .h 3 .c and one .vhd files
//   1-toto.h     external functions declarations
//   2-toto.c     containing filter implementation for integer and floating point numbers
//   3-toto_int.c,containing filter implementation only for integer numbers
//   4-toto_float.c,containing filter implementation only for floating points numbers
//   4-toto.vhd,containing filter implementation in vhdl
//------------------------------------------------------
   params.file_name="band_stop"; // will generate band_stop.h, band_stop.c,band_stop_int.c,band_stop_float.c,band_stop.vhd
//---------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
   params.name_filter="band_stop"; // specific functions for this filter will end with _band_stop
//--------------------------------------------------------------
...
// 3.1-use of function band_pass_en_p, as for a continuous filter
  [K,F_of_w] = band_stop_en_p(modele_flt,v0,v1,v2,v3,G03_db,G12_db);
