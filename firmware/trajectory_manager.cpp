...
void predictive_control_init() {
  pControl.i0 =
      0.00353;  // kg.m**2. Measured moment of inertia when empty (gear box)
  pControl.vAlim = 12;      // V
  pControl.r = 4.07;        // Ohm. Datasheet says 5.86 ohm but is not reliable
  pControl.ke = 1.399;      // V.s/rad (voltage/rotational speed).
                            /*
                             * The datasheet of the supposed motor from maxon says 694/200 rpm/V (the 200
                             * come from the gear box ratio) = 0.3634 rad/(s*V) => ke = 2.75 V*s/rad
                             * Btw the ke (in V*s/rad) == kt (in N.m/A) is valid in the datasheet since
                             * they give torqueConstant = 13.8*200 mN.m/A (the 200 come from the gear box
                             * ratio) = 2.76 N.m/A
                             * After manually counting the teeth of the gears, the actual gear box ratio
                             * is 172.08...
                             */
  pControl.kvis = -0.0811;  // in N.m.s/rad
  pControl.linearTransition = 0.195;  // in rad/s.
  pControl.kstat =
      0.1212;  // N.m. Minimum torque needed to start moving the motor
  pControl.kcoul =
      0.103; /* in N.m value of the friction torque when speed =
                linearTransition.
...
predictiveControl* get_predictive_control() { return &pControl; }
...
float traj_eval_poly(volatile float* pPoly, float* pTimePowers) {
  return pPoly[0] + pTimePowers[0] * pPoly[1] + pTimePowers[1] * pPoly[2] +
         pTimePowers[2] * pPoly[3] + pTimePowers[3] * pPoly[4];
}

float traj_eval_poly_derivate(volatile float* pPoly, float* pTimePowers) {
  return pPoly[1] + pTimePowers[0] * 2 * pPoly[2] +
         pTimePowers[1] * 3 * pPoly[3] + pTimePowers[2] * 4 * pPoly[4];
}
...
/**
 * The formula used here is u(t) = voltsToCommand*[ke * speedInRads +
 * torqueToVoltage*(outputTorque + accelTorque - frictionTorque)]
 *
 * Where :
 * -> voltsToCommand*ke*speedInRads is the command needed to maintain the
 * current speed if there was no friction
 * -> voltsToCommand*torqueToVoltage*torque is the command that will make the
 * motor create 'torque' (expressed in N.m)
 * -> frictionTorque is the torque created by the static friction, the coulomb
 * friction and the viscous friction
 * -> outputTorque is the actual torque that could be measured outside the motor
 * -> accelTorque  = I * a(t) is the torque needed to create an acceleration of
 * a(t) during dt, provided that 'outputTorque' is
 * either null or absorbed by the environment (which is typically the case when
 * it's used as a weight compensation)
...
void predictive_control_tick(motor* pMot, int32 pVGoal, uint32 pDt,
  ...
  float beta =
      exp(-abs(speedInRads / pControl.linearTransition));  // range [0, 1]
  float accelTorque =
      ((goalSpeedInRads - speedInRads) * (pControl.i0 + pIAdded) * 10000.0) /
      ((float)pDt);  // in N.m. dt is in 1/10 of a ms

  // kvis is negative in this convention
  float frictionTorque = speedInRads * pControl.kvis -
                         signV * (beta * pControl.kstat +
                                  (1 - beta) * pControl.coulombContribution);

  int16 u = pControl.voltsToCommand *
            (pControl.ke * speedInRads +
             pControl.torqueToVoltage * (accelTorque - frictionTorque));
  int16 uTorque =
      pControl.voltsToCommand * (pControl.torqueToVoltage * (pOutputTorque));
  ...
  pControl.estimatedSpeed =
      pVGoal; /* Would be better if we could get the real-life speed from time
               * to time to update this value.
               * This is no easy task since getting the speed from a derivate of
               * the position comes with the
...
/**
 * Estimates the torque that's outputed by the motor (with and without
 * accounting for friction), when the command is pCommand and the rotationnal
 * speed
 * is pSpeed.
 * Same formula as predictive_control_tick but this function is used only to
 * update the output torques. Torque units in in [N.m]
 *
 */
void predictive_update_output_torques(int32 pCommand, int32 pSpeed) {
  ...
  //    int16 u = pControl.voltsToCommand *(pControl.ke * speedInRads +
  //    pControl.torqueToVoltage * (accelTorque - frictionTorque +
  //    pOutputTorque));
  float electricalTorque =
      ((pCommand / pControl.voltsToCommand) - pControl.ke * speedInRads) /
      pControl.torqueToVoltage;  // == -frictionTorque + outputTorque
...
/**
 * The formula used here is u(t) = voltsToCommand*[ke * speedInRads +
 * torqueToVoltage*(outputTorque - frictionTorque)]
 *
 * Where :
 * -> voltsToCommand*ke*speedInRads is the command needed to maintain the
 * current speed if there was no friction
...
void predictive_control_anti_gravity_tick(motor* pMot, int32 pVGoal,
  ...
  /* The issue here is that our initial model states that the static friction
   *depends on the sign of the current speed
   * which is true as long as the speed is not null.
   * When the speed is null, the static friction will oppose itself to the
   *torque applied on the motor.
   **/
  ...
  // Un-comment this to emulate an old windows PC loading a program (quite
  // funny)
  ...
  int16 u = pControl.voltsToCommand *
            (pControl.ke * speedInRads +
             pControl.torqueToVoltage * (-frictionTorque + pOutputTorque));
