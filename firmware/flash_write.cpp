...
void flash_lock() { FLASH_BASE->CR = FLASH_CR_LOCK; }
 ...
 * It's closer to what the datasheets says, but has not been tested as much as
 * the above version.
 */
// bool flash_erase_page(unsigned int pageAddr)
//{
...
bool flash_write_word(unsigned int addr, unsigned int word) {
  ...
  volatile unsigned int hhWord =
      ((volatile unsigned int)word & 0xFFFF0000) >> 16;
