...
/**
   /!\ Using this function implies that all of the encoder you'll connect will
   share the same CLK pin and the same CS pin.
 */
void encoder_init_sharing_pins_mode(uint8 pClkPin, uint8 pCsPin);

/**
 * Adds an encoder to the array of encoders when you're in the sharing pins
 * mode.
 * You need to call encoder_init_sharing_pins_mode(uint8 pClkPin, uint8 pCsPin)
 * before this one.
 */
 ...
 * The function encoder_read_angles_sharing_pins_mode() needs to be called first
 * though.
 */
long encoder_get_angle(uint8 pEncoderId);

/**
 * Returns the encoder angle in an one shot fashion (no need to call init or
 * addEncoder).
