...
long encoder_read_angle_sequential(uint8 pDOPin, uint8 pCLKPin, uint8 pCSPin) {
  ...
  angle = (angle >>
           6);  // shift 18-digit angle right 6 digits to form 12-digit value
...
/**
   ...
   A timer was used to schedule interruptions every 1us which is the delay
   between every step of the
   reading procedure (thus freeing the Uc during the waiting moments).
   Unfortunately, this approach was not viable because the treatment of an
   interruption procedure every 1us was
   overwhelming for the Uc (whose clock was 72Mhz, thus allowing only 72 cycles
   before the next
   interruption).

   This approach would still be viable in a context where it is important to
   optimize the Uc load and
   where it is acceptable to wait longer to get the encoder's value (set the
   interruption period
...
void addEncoder(uint8 pTimerIndex,
    ...
    //timer->setPrescaleFactor(1); //prescale = 1 => freq = 72MHz, prescale =
65535 => freq = 1099 Hz
...
void encoderTick() {
    ...
    case 6:
        ...
        if (debug) {
            ...
            //INCn = statusbits & 4; // goes high if magnet moved towards IC
            LIN = statusbits & 8; // goes high for linearity alarm
            COF = statusbits & 16; // goes high for cordic overflow: data
invalid
            //OCF = statusbits & 32; // this is 1 when the chip startup is
finished.

            if (LIN) {
                isDataQuestionable = true;
                //SerialUSB.println("linearity alarm: magnet misaligned? Data
questionable.");
            } else if (COF) {
                isDataInvalid = true;
                //SerialUSB.println("cordic overflow: magnet misaligned? Data
invalid.");
            } else {
